...
//---------------------------------------------------------------------
// GLSL noise by Stefan Gustavson (stefan.gustavson@liu.se)
...
vec3 permutePoly(vec3 x) {return mod289((34.0 * x + 10.0) * x);}
...
vec2 cellular(vec3 P) {
	...
	vec3 p = permutePoly(Pi.x + vec3(-1.0, 0.0, 1.0));
	vec3 p1 = permutePoly(p + Pi.y - 1.0);
	vec3 p2 = permutePoly(p + Pi.y);
	vec3 p3 = permutePoly(p + Pi.y + 1.0);

	vec3 p11 = permutePoly(p1 + Pi.z - 1.0);
	vec3 p12 = permutePoly(p1 + Pi.z);
	vec3 p13 = permutePoly(p1 + Pi.z + 1.0);

	vec3 p21 = permutePoly(p2 + Pi.z - 1.0);
	vec3 p22 = permutePoly(p2 + Pi.z);
	vec3 p23 = permutePoly(p2 + Pi.z + 1.0);

	vec3 p31 = permutePoly(p3 + Pi.z - 1.0);
	vec3 p32 = permutePoly(p3 + Pi.z);
	vec3 p33 = permutePoly(p3 + Pi.z + 1.0);
...
//		x.xy = rotate(x.xy, random(vec2(ivec2(octaves * -2, -seed))) * 3.14159);
		...
		v += a * cellular(vec3(repeat * x + rand_offset)).r;
		max_a += a;
		s /= lacunarity;
		a *= persistence;
	}
