...
# Noise types
var noise_shaders := [
	preload("res://addons/extended_3d_texture_generator/tool/noise_shaders/p_noise.shader"),
	preload("res://addons/extended_3d_texture_generator/tool/noise_shaders/cellular_noise.shader"),
	preload("res://addons/extended_3d_texture_generator/tool/noise_shaders/curl_noise.shader"),
	]

func _ready() -> void:
	if get_tree().edited_scene_root != self:
		$VBoxContainer/Create.icon = get_icon("Texture3D", "EditorIcons")
		#$VBoxContainer/TextureRect.texture = $Preview.get_texture()
		$Preview/ColorRect.material = $Preview/ColorRect.material.duplicate()
		$Preview/ColorRect.material.shader = noise_shaders[0]
...
func _on_NoiseOptionButton_item_selected(index: int) -> void:
	match index :
		0:
			$Preview/ColorRect.material.shader = noise_shaders[0]
		1:
			$Preview/ColorRect.material.shader = noise_shaders[1]
		2:
			$Preview/ColorRect.material.shader = noise_shaders[2]
